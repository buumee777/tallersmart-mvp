...
import { Car, CalendarClock, ClipboardList, Plus, UserRound, Wrench, Search, ShieldCheck, Save, LogIn, Trash2, Pencil, Building2, LogOut, MailCheck, MapPin, Phone, Clock, UserPlus, Eye, EyeOff, CreditCard, CheckCircle2, Wallet, ChevronRight, FileText, Users, Shield, Home, Menu, X, DollarSign, BarChart3, CalendarDays, ListChecks } from 'lucide-react';
import './styles.css';
...
const seed = {
  ...
  appointments: [
    { id: 1, workshopId: 1, clientId: 1, plate: 'AB123CD', date: '2026-06-03', time: '09:30', reason: 'Control general y revisión de frenos' },
    { id: 2, workshopId: 1, clientId: 2, plate: 'AC456EF', date: '2026-06-05', time: '11:00', reason: 'Service 120.000 km' },
  ],
};
...
function PanelSummary({ admin, myClients, myWorks }) {
  return <div className="card"><div className="panel-title"><div><h2>Panel del taller</h2><p>Solo ves clientes cargados por {admin.name}.</p></div><ShieldCheck/></div><div className="shop-data"><p><MapPin size={14}/> {admin.address}</p><p><Phone size={14}/> {admin.phone}</p><p><Clock size={14}/> {admin.hours}</p><p><CreditCard size={14}/> Mercado Pago: {admin.mercadoPagoAlias || 'sin cargar'}</p><p><Wallet size={14}/> MODO: {admin.modoAlias || 'sin cargar'}</p></div><div className="stats"><div className="stat"><strong>{myClients.length}</strong><span>Clientes</span></div><div className="stat"><strong>{myClients.filter(c=>statusFor(c)==='Urgente').length}</strong><span>Urgentes</span></div><div className="stat"><strong>{myWorks.length}</strong><span>Trabajos</span></div></div></div>;
}

function AdminMenu({ active, setActive, open, setOpen }) {
  const items = [
    ['clientes','Clientes',<Users size={20}/>],
    ['trabajos','Trabajos realizados',<ListChecks size={20}/>],
    ['facturacion','Facturación',<BarChart3 size={20}/>],
    ['turnos','Turnos',<CalendarDays size={20}/>],
  ];
  return <><button className="menu-trigger" onClick={()=>setOpen(true)}><Menu size={22}/> Menú</button>{open&&<div className="menu-backdrop" onClick={()=>setOpen(false)}><div className="menu-panel" onClick={e=>e.stopPropagation()}><div className="between"><h3>Menú del taller</h3><button className="icon-button" onClick={()=>setOpen(false)}><X size={20}/></button></div><div className="menu-options">{items.map(([key,label,icon])=><button key={key} className={active===key?'active':''} onClick={()=>{setActive(key); setOpen(false);}}>{icon}<span>{label}</span><ChevronRight size={18}/></button>)}</div></div></div>}</>;
}

function ClientTools({ myClients, selected, setSelected, editing, setEditing, saveClient, del, q, setQ, filtered, setSection }) {
  const [view,setView]=useState('home');
  return <div className="card"><h3>Clientes</h3>{view==='home'&&<div className="quick-actions"><button className="primary full" onClick={()=>{setEditing(null); setView('new')}}><Plus size={18}/> Nuevo cliente</button><button className="outline full" onClick={()=>setView('search')}><Search size={18}/> Buscar cliente</button></div>}{view==='new'&&<><button className="outline small" onClick={()=>setView('home')}>Volver</button><ClientForm editing={null} onSave={(client)=>{saveClient(client); setView('search')}} onCancel={()=>setView('home')}/></>}{view==='search'&&<><button className="outline small" onClick={()=>setView('home')}>Volver</button><div className="search client-search"><Search/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar por patente, cliente o vehículo"/></div><div className="client-list">{filtered.map(c=><button key={c.id} className={`client-item ${selected?.id===c.id?'selected':''}`} onClick={()=>setSelected(c)}><div><strong>{c.name}</strong><span className={badgeClass(statusFor(c))}>{statusFor(c)}</span></div><p>{c.vehicle}</p><small>Patente {c.plate}</small></button>)}</div>{selected&&<div className="selected-client-tools"><h4>{selected.name} · {selected.plate}</h4><div className="actions"><button className="outline" onClick={()=>setEditing(selected)}><Pencil size={16}/> Editar</button><button className="primary" onClick={()=>setSection('detalle')}><Wrench size={16}/> Cargar trabajo</button><button className="danger" onClick={()=>del(selected.id)}><Trash2 size={16}/> Eliminar</button></div></div>}{editing&&<ClientForm editing={editing} onSave={saveClient} onCancel={()=>setEditing(null)}/>}</>}</div>;
}

function WorksList({ works, data }) {
  const [open,setOpen]=useState(null);
  return <div className="card"><h3>Trabajos realizados</h3><p>Listado completo del taller. Tocá un trabajo para consultar estado de cobro y detalle.</p><div className="history">{works.length===0&&<p>No hay trabajos realizados.</p>}{works.map(w=>{ const client=data.clients.find(c=>cleanPlate(c.plate)===cleanPlate(w.plate) && c.workshopId===w.workshopId) || data.clients.find(c=>cleanPlate(c.plate)===cleanPlate(w.plate)); const isOpen=open===w.id; return <div className="history-item" key={w.id}><button className="work-button" onClick={()=>setOpen(isOpen?null:w.id)}><div><strong>{w.title}</strong><p>{client?.name || 'Cliente no identificado'} · Patente {w.plate}</p><small>{w.date} · {Number(w.km||0).toLocaleString('es-AR')} km</small></div><div className="history-side"><strong>{money(w.cost)}</strong>{w.paymentStatus==='paid'?<span className="payment-paid"><CheckCircle2 size={16}/> Pago acreditado</span>:<span className="payment-pending"><CreditCard size={16}/> Pago pendiente</span>}</div></button>{isOpen&&<div className="work-detail"><p><b>Cliente:</b> {client?.name || '-'}</p><p><b>Vehículo:</b> {client?.vehicle || '-'}</p><p><b>Pago:</b> {w.paymentStatus==='paid' ? `Acreditado ${formatPaidDate(w.paidAt)}` : 'Pendiente de cobro'}</p></div>}</div>})}</div></div>;
}

function BillingPanel({ works }) {
  const paid=works.filter(w=>w.paymentStatus==='paid');
  const pending=works.filter(w=>w.paymentStatus!=='paid');
  const total=works.reduce((a,w)=>a+parseAmount(w.cost),0);
  const paidTotal=paid.reduce((a,w)=>a+parseAmount(w.cost),0);
  const pendingTotal=pending.reduce((a,w)=>a+parseAmount(w.cost),0);
  const months={};
  works.forEach(w=>{ const key=(w.date||'Sin fecha').slice(0,7); months[key]=(months[key]||0)+parseAmount(w.cost); });
  const years={};
  works.forEach(w=>{ const key=(w.date||'Sin fecha').slice(0,4); years[key]=(years[key]||0)+parseAmount(w.cost); });
  return <div className="card"><h3>Facturación</h3><p>Métricas estimadas según trabajos cargados en el demo.</p><div className="billing-grid"><div className="billing-box"><span>Total facturado</span><strong>${total.toLocaleString('es-AR')}</strong></div><div className="billing-box"><span>Cobrado</span><strong>${paidTotal.toLocaleString('es-AR')}</strong></div><div className="billing-box"><span>Pendiente</span><strong>${pendingTotal.toLocaleString('es-AR')}</strong></div><div className="billing-box"><span>Trabajos</span><strong>{works.length}</strong></div></div><h4>Mensual</h4><div className="metric-list">{Object.entries(months).map(([k,v])=><div key={k}><span>{k}</span><b>${v.toLocaleString('es-AR')}</b></div>)}</div><h4>Anual</h4><div className="metric-list">{Object.entries(years).map(([k,v])=><div key={k}><span>{k}</span><b>${v.toLocaleString('es-AR')}</b></div>)}</div><h4>Por trabajo</h4><div className="metric-list">{works.map(w=><div key={w.id}><span>{w.title}</span><b>{money(w.cost)}</b></div>)}</div></div>;
}

function AppointmentForm({ clients, onSave }) {
  const [f,setF]=useState({clientId:clients[0]?.id || '', date:'', time:'', reason:''});
  const set=(k,v)=>setF({...f,[k]:v});
  const save=()=>{ const client=clients.find(c=>String(c.id)===String(f.clientId)); if(!client || !f.date || !f.time) return alert('Seleccioná cliente, fecha y horario.'); onSave({id:Date.now(), workshopId:client.workshopId, clientId:client.id, plate:cleanPlate(client.plate), date:f.date, time:f.time, reason:f.reason || 'Turno de mantenimiento'}); setF({clientId:clients[0]?.id || '', date:'', time:'', reason:''}); };
  return <div className="card"><h3>Asignar turno</h3><label className="field"><span>Cliente registrado</span><select value={f.clientId} onChange={e=>set('clientId',e.target.value)}>{clients.map(c=><option key={c.id} value={c.id}>{c.name} · {c.plate} · {c.vehicle}</option>)}</select></label><div className="grid2"><Field label="Fecha" type="date" value={f.date} onChange={v=>set('date',v)}/><Field label="Hora" type="time" value={f.time} onChange={v=>set('time',v)}/></div><Field label="Motivo" value={f.reason} onChange={v=>set('reason',v)} placeholder="Ej: cambio de aceite"/><button className="primary" onClick={save}><CalendarClock size={18}/> Guardar turno</button></div>;
}

function AppointmentList({ appointments, clients }) {
  return <div className="card"><h3>Turnos asignados</h3><p>Ordenados por fecha de turno.</p><div className="history">{appointments.length===0&&<p>No hay turnos asignados.</p>}{appointments.map(t=>{ const c=clients.find(x=>x.id===t.clientId); return <div className="appointment-item" key={t.id}><div><strong>{t.date} · {t.time}</strong><p>{c?.name || 'Cliente'} · {t.plate}</p><small>{t.reason}</small></div><CalendarDays/></div>})}</div></div>;
}

function TurnsPanel({ clients, appointments, onSave }) {
  const [view,setView]=useState('home');
  return <div className="card"><h3>Turnos</h3>{view==='home'&&<div className="quick-actions"><button className="primary full" onClick={()=>setView('assign')}><Plus size={18}/> Asignar turno</button><button className="outline full" onClick={()=>setView('list')}><CalendarDays size={18}/> Consultar turnos asignados</button></div>}{view==='assign'&&<><button className="outline small" onClick={()=>setView('home')}>Volver</button><AppointmentForm clients={clients} onSave={onSave}/></>}{view==='list'&&<><button className="outline small" onClick={()=>setView('home')}>Volver</button><AppointmentList appointments={appointments} clients={clients}/></>}</div>;
}

function AdminPanel({ data, setData, admin, refreshAdmin }) {
  const myClients = data.clients.filter(c=>c.workshopId===admin.id);
  const myWorks = data.works.filter(w=>w.workshopId===admin.id).sort((a,b)=>new Date(b.date)-new Date(a.date));
  const myAppointments = (data.appointments || []).filter(t=>t.workshopId===admin.id).sort((a,b)=>`${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`));
  const [selected,setSelected]=useState(myClients[0]||null);
  const [editing,setEditing]=useState(null);
  const [q,setQ]=useState('');
  const [menuOpen,setMenuOpen]=useState(false);
  const [section,setSection]=useState('detalle');
  ...
  const addAppointment=(appointment)=>setData({...data, appointments:[appointment,...(data.appointments||[])]});
  ...
  return <main className="layout"><section className="sidebar"><div className="card admin-top"><div className="between"><div><h2>Panel del taller</h2><p>{admin.name}</p></div><AdminMenu active={section} setActive={setSection} open={menuOpen} setOpen={setMenuOpen}/></div></div><PanelSummary admin={admin} myClients={myClients} myWorks={myWorks}/></section><section className="detail">{section==='detalle'&& (selected ? <VehicleDetail client={selected} works={works} data={data} admin onEdit={()=>{setEditing(selected); setSection('clientes')}} onDelete={()=>del(selected.id)} onAddWork={addWork}/> : <div className="card">Cargá o seleccioná un cliente desde el menú Clientes.</div>)}{section==='clientes'&&<ClientTools myClients={myClients} selected={selected} setSelected={setSelected} editing={editing} setEditing={setEditing} saveClient={saveClient} del={del} q={q} setQ={setQ} filtered={filtered} setSection={setSection}/>} {section==='trabajos'&&<WorksList works={myWorks} data={data}/>} {section==='facturacion'&&<BillingPanel works={myWorks}/>} {section==='turnos'&&<TurnsPanel clients={myClients} appointments={myAppointments} onSave={addAppointment}/>}</section></main>;
}
