...
import './styles.css';
const VERSION = 'TallerSmart Real MVP v11.0.06 · IA Real Demo';
...
function ClientAccount({signOut}){
  ...
  async function addVehicle(){
    ...
    if(!normalized){showError('Ingresá una patente.');return}
    const isPremium=(profile?.plan||'free')!=='free';
    const limit=Number(profile?.vehicle_limit || 1);
    if(!isPremium && vehicles.length>=limit){showError('Tu plan gratis permite 1 vehículo. Para agregar más, activá Cliente PRO.');return}
    setMsgType('info'); setMsg('Asociando vehículo...');
    ...
    if(!v){
      ...
      if(createErr){showError(createErr.message);return}
      v=newV;
    }
    const {error:linkErr}=await supabase.from('user_vehicles').insert({vehicle_id:v.id,plate:normalized,alias:normalized});
    if(linkErr){showError(linkErr.message);return}
    setPlate(''); showSuccess('Vehículo asociado correctamente.'); await loadGarage(); setSelected(v); setScreen('home');
  }
  ...
  async function activateProDemo(){
    ...
    const next={...profile,plan:'pro',vehicle_limit:10};
    setProfile(next);
    showSuccess('PRO Demo activado correctamente. Ya podés probar funciones premium.');
    setScreen('home');
  }
  ...
  return <main className="client client-session">
    ...
    {screen==='vehicles'&&<><button className="link" onClick={()=>setScreen('home')}><ArrowLeft/>Volver</button><h2>Agregar vehículo</h2><div className="plan-card"><b>{planFree?'Plan gratis':'Cliente PRO'}</b><small>{planFree?`Usados ${vehicles.length}/1 vehículos. Para más vehículos necesitás PRO.`:'Podés asociar múltiples vehículos.'}</small></div><input placeholder="Patente" value={plate} onChange={e=>setPlate(e.target.value)}/><button onClick={addVehicle}>Asociar patente</button>{planFree&&<button onClick={activateProDemo}><Crown/>Activar PRO Demo</button>}</>}
...
function ClientAI({selected,jobs,onBack}){
  ...
  async function analyze(){
    setError(''); setAnswer('');
    if(!symptoms.trim()){setError('Escribí qué sentís, escuchás o ves en el vehículo.'); return;}
    setLoading(true);
    try{
      const res = await fetch('/api/vehicle-ai',{
        ...
        body:JSON.stringify({
          symptoms,
          vehicle:selected ? {plate:selected.plate,brand:selected.brand,model:selected.model,year:selected.year,current_km:selected.current_km,vtv_expiration_date:selected.vtv_expiration_date} : null,
          jobs: jobs.slice(0,8).map(j=>({title:j.title,description:j.description,km:j.km,cost:j.cost,payment_status:j.payment_status,job_date:j.job_date,workshop:j.workshops?.name}))
        })
      });
  ...
  return <Screen title="IA vehicular" onBack={onBack}>
    <div className="premium-card ai-card"><Brain/><h3>Diagnóstico inteligente</h3><p>Describí el problema y la IA te dará una orientación inicial usando los datos del vehículo y su historial.</p><textarea value={symptoms} onChange={e=>setSymptoms(e.target.value)} placeholder="Ej: hace ruido al frenar, vibra en ruta, prende una luz del tablero..."/><button onClick={analyze} disabled={loading}><Brain/> {loading?'Analizando...':'Analizar con IA'}</button></div>
    {error&&<div className="alert-red"><AlertTriangle/><b>{error}</b></div>}
    {answer&&<div className="ai-answer"><CheckCircle2/><h3>Respuesta de IA</h3><p>{answer}</p><small>Orientación inicial. No reemplaza el diagnóstico de un mecánico profesional.</small></div>}
    {!answer&&<div className="item"><b>Modo PRO</b><small>Disponible para Cliente PRO / PRO Demo. La respuesta puede incluir posibles causas, nivel de urgencia y próximos pasos sugeridos.</small></div>}
  </Screen>
}
