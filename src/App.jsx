...
const VERSION = 'TallerSmart Real MVP v8.0';
...
const mapsUrl = (w) => { const q = [w?.address, w?.name].filter(Boolean).join(' '); return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q || 'taller mecánico')}`; };
...
function ClientPlate({signOut}){
  ...
  async function search(value=plate, silent=false){
    ...
    const {data:j,error:jErr}=await supabase.from('jobs').select('*, workshops(name,address,phone,opening_hours)').eq('vehicle_id',v.id).order('job_date',{ascending:false});
    if(jErr){ setMsg(jErr.message); return; }
    const {data:a,error:aErr}=await supabase.from('appointments').select('*, workshops(name,address,phone,opening_hours), clients(name)').eq('vehicle_id',v.id).order('appointment_date',{ascending:true});
  ...
  async function pay(job){ await supabase.from('jobs').update({payment_status:'paid'}).eq('id',job.id); await search(plate, true); setSelectedJob(null); }
...
function JobDetail({job,onBack,onPay}){
  ...
  return <div className="job"><button className="link" onClick={onBack}><ArrowLeft/>Volver a trabajos</button><h3>{job.title}</h3><p>{job.description||'Sin detalle cargado.'}</p><small>{formatDate(job.job_date)} · {job.km||0} km · {fmt(job.cost)}</small><div className={job.payment_status==='paid'?'paid':'pending'}>{job.payment_status==='paid'?<><CheckCircle2/> Pago acreditado</>:<><Clock3/> Pendiente de pago</>}</div>{job.payment_status!=='paid'&&<button onClick={()=>onPay(job)}><CreditCard/> Pagar demo</button>}<section className="workshop-box"><h3>Datos del taller</h3><p><b>{w.name||'Taller no informado'}</b></p><p>{w.address||'Dirección no informada'}</p><a href={mapsUrl(w)} target="_blank" rel="noreferrer"><MapPin/> Abrir en Google Maps</a><a href={whatsappUrl(w.phone)} target="_blank" rel="noreferrer"><Phone/> WhatsApp del taller</a><p><b>Horario:</b> {w.opening_hours||'No informado'}</p></section></div>
}
